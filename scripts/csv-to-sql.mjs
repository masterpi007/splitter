...
//
// D1 rejects SQL BEGIN TRANSACTION, so the output is a plain statement list.
// Each file starts with a commented pre-flight query that proves every member
// name resolves, plus the one-line DELETE that undoes the import.
...
function emit(list) {
  const out = [];
  const names = [...new Set(list.flatMap((x) => [x.paidBy, ...x.parts.map((p) => p.name)]))];
  out.push('-- generated by scripts/csv-to-sql.mjs — review before running');
  out.push('--');
  // D1 rejects BEGIN TRANSACTION (it is a Durable Object; transactions are a
  // JS-level API), so these statements are not atomic. Run the pre-flight
  // query first: it proves every name resolves, which is the only realistic
  // way this import fails midway.
  out.push('-- STEP 1 — pre-flight, run this alone and expect found = 1 on every row:');
  out.push('--');
  out.push(
    '--   SELECT n.name, (SELECT count(*) FROM members m WHERE m.group_id = ' +
      q(groupId) +
      ' AND lower(m.name) = lower(n.name) AND m.removed_at IS NULL) AS found FROM (',
  );
  names.forEach((n, i) => {
    out.push(`--     ${i === 0 ? 'SELECT' : 'UNION ALL SELECT'} ${q(n)} AS name`);
  });
  out.push('--   ) n;');
  out.push('--');
  out.push('-- STEP 2 — run everything below.');
  out.push('--');
  out.push('-- Undo (removes only this import; children cascade):');
  out.push(`--   DELETE FROM expenses WHERE group_id = ${q(groupId)};`);
  out.push('');
